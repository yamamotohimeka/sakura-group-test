    <section class="newface">
      ...
      <div class="shop-switch">
        ...
        <?php foreach ($groupShopArray as $key => $value) { ?>
          <a href="/newface/<?php echo $key; ?>" class="item"><span class="text<?php if ($shop_id == $key) echo ' yellow"'; ?>"><?php echo $value['name']; ?></span></a>
        <?php } ?>
      </div>
        ...
        <ul class="girl-list">
          <?php foreach ($printArray['GirlNewface'] as $key => $value) { ?>
            <li class="girl<?php if ($value['ScheduleStatus']['status'] == '只今出勤中') echo ' attending'; ?>">
              ...
              <?php } ?>
              <div class="girl-meta">
              	<?php if(!empty($value['Girl']['started_at'])){ ?><div class="entry-date"><?php $started_at = new DateTime($value['Girl']['started_at']); echo $started_at->format('m月d日'); ?>入店</div><?php } ?>
